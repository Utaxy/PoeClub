...
const Messages = ()=>{
  ...
  <div
    key={message.id}
    className="flex flex-col border border-white w-full max-w-5xl rounded-lg bg-neutral-800/50 backdrop-blur-sm overflow-hidden
               max-h-[400px] md:max-h-[520px] lg:max-h-[640px]"
  >
    ...
    <div className="p-3 md:p-6 lg:p-8 flex-1 overflow-auto">
      {loggedUser ? (
        <>
          ...
          {/* Comments list - limited height with its own internal scroll if needed */}
    ...
    {/* Footer / actions stay visible and do not scroll away */}
    ...
    {/* Inline comment form (optional) - will appear above footer */}
    {openCommentFor === message.id && (
      <form onSubmit={(e) => commentSubmit(message.id, e)} className="p-3 md:px-6 md:py-4 border-t border-white/10 bg-neutral-900/30">
        <div className="flex flex-col gap-3">
          <div className="text-sm text-red-300">{notify}</div>
          <input
